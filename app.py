...
with col2:
    ...
    donut_chart = alt.Chart(response_data).mark_arc(innerRadius=60).encode(
        ...
        color=alt.Color('Attrition:N', scale=alt.Scale(domain=['Left', 'Stayed'], range=['#339999', '#4682B4']),
...
with col7:
    ...
    stacked_cat_chart = alt.Chart(df).mark_bar().encode(
        ...
        color=alt.Color('Attrition', scale=alt.Scale(domain=['Left', 'Stayed'], range=['#339999', '#4682B4']),
                    legend=alt.Legend(orient="top", direction="horizontal")),
