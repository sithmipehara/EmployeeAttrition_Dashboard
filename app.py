...
with col1:
    ...
    # Displaying labels
    st.markdown(f"<h5 style='text-align: left;'>Stayed<br> {stayed_count}<br> ({stayed_percentage:.1f}%)</h5>", unsafe_allow_html=True)
    st.markdown(f"<h5 style='text-align: right;'>Left<br> {left_count} <br>({left_percentage:.1f}%)</h5>", unsafe_allow_html=True)
